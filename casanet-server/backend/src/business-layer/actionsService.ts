...
export class ActionsService {
  ...
  constructor(private actionsDal: ActionsDal, private minionsService: MinionsBl) {
    ...
    this.minionsService.minionFeed.attach(minionFeed => {
      this.onMinionStatusChange(minionFeed);
    });
    // Run in interval to force the action set, in case of ActionApply permanent.
    setInterval(() => {
      this.forceActionsSet();
    }, ACTIONS_ACTIVATION.Milliseconds);
  }
  ...
  private async detectActionTrigger(action: Action, forceSet: boolean) {
    ...
    logger.info(
      `[ActionsService.applyAction] action ${action.actionId} "${action.name}" triggered due to minion ${
        minion.minionId
  ...
  private async deleteMinionActions(minion: Minion) {
    logger.info(
      `[ActionsService.deleteMinionActions] Collecting all "${minion.minionId}" minion's actions in order to delete them all`,
    );
  ...
  private async triggerMinionStatusChangedActions(minion: Minion) {
    ...
    logger.info(
      `[ActionsService.triggerMinionStatusChangedActions] Triggering minion ${minion.minionId} ${minionActions?.length} actions..."`,
    );

    // Detect all of them if need to be triggered
    for (const minionAction of minionActions) {
      logger.info(
        `[ActionsService.triggerMinionStatusChangedActions] Triggering minion ${minion.minionId} action ${minionAction.actionId}..."`,
      );
      await this.detectActionTrigger(minionAction, true);
    }
    logger.info(
      `[ActionsService.triggerMinionStatusChangedActions] Triggering minion ${minion.minionId} actions done"`,
    );
  ...
  private async validateActionParams(action: Action) {
    ...
    if (action.thenSet.find(set => set.minionId === action.minionId)) {
      logger.error(
        `[ActionsService.validateActionParams] Action is invalid, unable to set minion ${action.minionId} as trigger to it self`,
      );
      throw {
        responseCode: 11405,
    ...
    if (!ifStatus) {
      logger.error(
        `[ActionsService.validateActionParams] Action trigger validation failed, no valid status for minion ${
          minion.minionId
        } in status "${JSON.stringify(action.ifStatus)}"`,
      );
      throw {
        responseCode: 8405,
    ...
    for (const actionSet of action.thenSet) {
      ...
      if (!setStatus) {
        logger.error(
          `[ActionsService.validateActionParams] Action set validation failed, no valid status for minion ${
            actionSet.minionId
          } in status "${JSON.stringify(actionSet.setStatus)}"`,
        );
        throw {
          responseCode: 9405,
