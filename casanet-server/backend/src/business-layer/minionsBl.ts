...
export class MinionsBl {
  ...
  private async scanMissingDevices() {
    ...
    while (true) {
      ...
      logger.info(
        `[MinionsBl.scanMissingDevices] Minions "${missingDevices
          .map(m => m.minionId)
          .join(',')}" does not have yet IP, about to scan again...`,
      );
      ...
      try {
        ...
        for (const minion of missingDevices) {
          ...
          try {
            ...
            logger.error(
              `[MinionsBl.scanMissingDevices] Failed to read minions "${minion.minionId}" status "${error.message}" `,
            );
          }
        }
      } catch (error) {
  ...
  private async onMinionUpdated(minion: Minion, updateToStatus: MinionStatus) {
    ...
    if (
      minion.isProperlyCommunicated &&
      isequal(minion.minionStatus?.[minion.minionType], updateToStatus?.[minion.minionType])
    ) {
      return;
    }
