...
export class DevicesService {
  ...
  constructor(
    ...
    this.modulesManager.deviceStatusChangedEvent?.attach?.(deviceUpdate => {
      for (const device of this.localDevices) {
        if (device.mac === deviceUpdate.mac) {
          logger.info(
            `[DevicesService] Updating device "${
              device.mac
            }" due to update from driver with new status "${JSON.stringify(deviceUpdate.status)}"`,
          );
    ...
    setInterval(async () => {
      logger.info(
        `[DevicesService] About to scan netwrok changes as activation each ${DETECT_NETWORK_CHANGES_ACTIVATION}`,
      );
      try {
