...
export class TimingsBl {
  ...
  private async deleteMinionTimings(minion: Minion) {
    logger.info(
      `[TimingsService.deleteMinionTimings] Collecting all "${minion.minionId}" minion's timings in order to delete them all`,
    );
  ...
  private async activeTiming(timing: Timing): Promise<void> {
    ...
    try {
      ...
      if (timing.triggerDirectAction) {
        logger.info(
          `[activeTiming] Invoking id: ${timing.timingId} as trigger by direct action for minion "${timing.triggerDirectAction.minionId}" ...`,
        );
        results = await this.operationBl.triggerOperationActivities([timing.triggerDirectAction], {
          overrideLock,
          setLock,
        });
      } else {
