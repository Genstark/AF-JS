...
import { Minion, MinionFeed, MinionChangeTrigger, MinionStatus, SwitchOptions } from '../models/sharedInterfaces';
  ...
  public async initCalibrateModule() {
    ...
    this.minionsBl.minionFeed.attach(async (minionFeed: MinionFeed) => {
      ...
      if (minionFeed.trigger === 'device' && minion?.calibration?.calibrationMode === 'LOCK_DASHBOARD') {
        ...
        try {
          logger.debug(
            `[CalibrateBl.calibrateMinion] Minion "${
              minion.minionId
            }" is locked for physical changes status "${JSON.stringify(minionFeed.oldMinion.minionStatus)}" ...`,
          );
          ...
          logger.debug(
            `[CalibrateBl.calibrateMinion] Undo minion ${minion.minionId} physical changes successfully done`,
          );
        } catch (error) {
          logger.warn(
            `[CalibrateBl.calibrateMinion] Calibrate minion ${minion.minionId} fail, ${JSON.stringify(error)}`,
          );
        }
        return;
      }
