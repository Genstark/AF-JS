...
export class CommandsCacheManager extends CacheManager {
  ...
  public async getIrCommand(minion: Minion, setStatus: MinionStatus): Promise<string | string[] | ErrorResponse> {
    ...
    if (minionCache.acCommands.on && minionCache?.lastStatus?.airConditioning?.status === 'off') {
      if (typeof hexCommandCode === 'string') {
        hexCommandCode = [hexCommandCode];
      }
      const onCommand =
        typeof minionCache.acCommands.on === 'string' ? [minionCache.acCommands.on] : minionCache.acCommands.on;
