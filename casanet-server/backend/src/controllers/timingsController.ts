...
export class TimingsController extends Controller {
  ...
  @MinionsRestriction({
    ...
    elementArgIndex: 0,
    extractMinionIds: async (timingId: string) =>
      (await TimingsBlSingleton.getTimingById(timingId))?.triggerDirectAction?.minionId,
  })
  @Get('{timingId}')
  ...
  @MinionsRestriction({
    requirePermission: 'WRITE',
    elementArgIndex: 1,
    extractMinionIds: (timing: Timing) => timing.triggerDirectAction?.minionId,
  })
  @Put('{timingId}')
