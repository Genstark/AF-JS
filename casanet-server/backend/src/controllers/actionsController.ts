...
export class ActionsController extends Controller {
  ...
  @MinionsRestriction({
    ...
    elementArgIndex: 0,
    extractMinionIds: async (actionId: string) => (await actionsService.getActionById(actionId)).minionId,
  })
  @Get('{actionId}')
  ...
  @MinionsRestriction({
    requirePermission: 'READ',
    elementArgIndex: 0,
    extractMinionIds: (minionId: string) => minionId,
  })
  @Get('/minion/{minionId}')
  ...
  @MinionsRestriction({
    requirePermission: 'WRITE',
    elementArgIndex: 1,
    extractMinionIds: (action: Action) => action.minionId,
  })
  @Put('{actionId}')
  ...
  @MinionsRestriction({
    requirePermission: 'WRITE',
    elementArgIndex: 0,
    extractMinionIds: async (action: Action) => action.minionId,
  })
  @Post()
