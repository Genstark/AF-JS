...
export class AuthController extends Controller {
  private activeSession(key: string) {
    ...
    this.setHeader(
      'Set-Cookie',
      `${SESSION_COOKIE_NAME}=${key}; Max-Age=${maxAgeInSec}; Path=/; HttpOnly; ${
        Configuration.http.useHttps ? 'Secure' : ''
      }; SameSite=Strict;`,
    );
  }
  ...
  public async login(@Request() request: express.Request, @Body() login: Login): Promise<void> {
    ...
    try {
      ...
      if (loginResults.error) {
        this.setStatus(403);
        return (
          loginResults?.error?.responseCode && (({ responseCode: loginResults.error.responseCode } as unknown) as void)
        );
      }
    } catch (error) {
      this.setStatus(403);
