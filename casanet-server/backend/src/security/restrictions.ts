 ...
 */
async function validateMinionRestriction(
  user: User,
  minionId: string,
  requirePermission: RestrictionType,
): Promise<RestrictionItem | undefined> {
  ...
  if (
    restriction.restrictionType === 'WRITE' ||
    (requirePermission === 'READ' && restriction.restrictionType === 'READ')
  ) {
    return restriction;
  }
 ...
 */
export const MinionsRestriction = (options: MinionsRestrictionOptions) => (
  target: any,
  key: string,
 ...
 */
export const MinionsResultsRestriction = <T>(extractMinionId: (data: T) => string) => (
  target: any,
  key: string,
  descriptor: PropertyDescriptor,
) => {
  ...
  descriptor.value = async function(...args: any[]) {
    ...
    for (const item of result) {
      ...
      try {
        ...
        logger.warn(
          `[MinionsResultsRestriction] Failed to validate minion "${minionId}" restriction, skipping adding to collection err: ${error?.message}`,
        );
      }
    }
