...
export class BroadlinkHandler extends BrandModuleBase {
  ...
  private async getBroadlinkInstance(minion: Minion): Promise<Device | ErrorResponse> {
    try {
      ...
      logger.info(
        `[BroadlinkModule.getBroadlinkInstance] Devices founded ${list.map(i => toNormalMac(i.mac)).join(',')}`,
      );
      ...
      if (!device) {
        logger.error(
          `[BroadlinkModule.getBroadlinkInstance] Device ${minion.minionId} ${minion.device.pysicalDevice.mac} not discovered`,
        );
        throw {
          responseCode: 1503,
      ...
      logger.error(
        `[BroadlinkModule.getBroadlinkInstance] Device ${minion.minionId} ${minion.device.pysicalDevice.mac} Commination failed $`,
      );
      throw {
        responseCode: 1503,
  ...
  /** Set broadlink power */
  private async setBroadlinkPowerMode(broadlink: Sp2, switchOptions: SwitchOptions): Promise<void | ErrorResponse> {
    try {
  ...
  private async setIrAcStatus(minion: Minion, setStatus: MinionStatus): Promise<void | ErrorResponse> {
    ...
    for (let index = 0; index < RESEND_BEAM_COMMAND_TIMES; index++) {
      logger.debug(
        `[broadlinkHandler.fetchDevicesStateInterval] Sleeping a second before re-sending beam command on the ${index}/${RESEND_BEAM_COMMAND_TIMES} attempts`,
      );
