...
export class MqttHandler extends BrandModuleBase {
  ...
  private async loadMqttDrivers() {
    ...
    ///////////////////////////////////////////
    this.mqttDrivers.push(
      new CasanetMqttDriver(this.deviceStatusChangedEvent, this.retrieveMinions, this.minionStatusChangedEvent),
    );
    this.mqttDrivers.push(
      new ShellyMqttDriver(this.deviceStatusChangedEvent, this.retrieveMinions, this.minionStatusChangedEvent),
    );
    this.mqttDrivers.push(
      new TasmotaMqttDriver(this.deviceStatusChangedEvent, this.retrieveMinions, this.minionStatusChangedEvent),
    );
    ...
    this.mqttClient.on('message', async (topic: string, payload: Buffer) => {
      try {
        ...
        if (!parsedMqttMessage.minion) {
          logger.warn(
            `[MqttHandler.onMessage] Fail to update minion status from MQTT message, minion/device not exist`,
          );
          return;
        }
        ...
        if (!parsedMqttMessage.minionStatus[parsedMqttMessage.minion.minionType]) {
          logger.warn(
            `[MqttHandler.onMessage] Message topic ${topic} for minion ${parsedMqttMessage.minion.minionId} contained invalid status payload "${messageData}", aborting...`,
          );
          return;
        }
        ...
        logger.error(
          `[MqttHandler.onMessage] Parsing mqtt message for ${topic}:${payload?.toString?.()} failed, error: ${JSON.stringify(
            error,
          )}`,
        );
      }
