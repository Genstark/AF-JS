...
export class TasmotaMqttDriver extends MqttBaseDriver {
  ...
  public deviceTopics = ['stat/+/RESULT'];
  ...
  public convertSetStatusMessage(minion: Minion, setStatus: MinionStatus): MqttMessage[] {
    ...
    if (minion.minionType === 'colorLight' && setStatus?.colorLight?.status === 'on') {
      ...
      if (
        setLight.red !== originalLight?.red ||
        ...
        baseMqttMessage.push({
          ...
          data: `${setLight.red.toString(16).padStart(2, '0')}${setLight.green
            .toString(16)
            .padStart(2, '0')}${setLight.blue.toString(16).padStart(2, '0')}`,
        });
      }
    ...
    if (minion?.minionType === 'switch') {
      ...
      minionStatus = {
        colorLight: {
          ...
          red:
            (!asJson?.Color ? minion?.minionStatus?.colorLight?.red : parseInt(asJson.Color.substring(0, 2), 16)) || 1,
          green:
            (!asJson?.Color ? minion?.minionStatus?.colorLight?.green : parseInt(asJson.Color.substring(2, 4), 16)) ||
            1,
          blue:
            (!asJson?.Color ? minion?.minionStatus?.colorLight?.blue : parseInt(asJson.Color.substring(4, 6), 16)) || 1,
        },
      };
    }
