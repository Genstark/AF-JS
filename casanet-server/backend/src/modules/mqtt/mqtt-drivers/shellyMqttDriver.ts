...
export class ShellyMqttDriver extends MqttBaseDriver {
  ...
  constructor(
    deviceFeed: SyncEvent<{
      deviceId: string;
      status: DeviceStatus;
    }>,
    retrieveMinions: PullBehavior<Minion[]>,
    minionStatusChangedEvent: SyncEvent<{
      minionId: string;
      status: MinionStatus;
    }>,
  ) {
  ...
  private async actionsListener(req, res) {
    ...
    logger.info(
      `[ShellyMqttDriver.actionsListener] minion ${minion.minionId} ${minion.name} status update via Shelly actions to ${status}`,
    );
  ...
  public async convertMqttMessage(topic: string, data: string): Promise<ParsedMqttMessage> {
    ...
    if (deviceType === 'relay') {
      // case of smart switch as MP1
      minion = minions.find(
        m => m?.device?.deviceId === deviceId && m?.device?.brand === this.brandName && m?.device?.model === 'MP1',
      );
      minionStatus = {
        switch: {
    ...
    } else if (deviceType === 'ext_temperature') {
      // Case of temp. sensor
      ...
      minion = minions.find(
        m =>
          ...
          m?.device?.brand === this.brandName &&
          m?.device?.model === `MP1 with sensor extension - the ${modelExt}`,
      );
