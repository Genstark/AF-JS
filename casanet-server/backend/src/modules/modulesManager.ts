...
export class ModulesManager {
  ...
  public async setStatus(minion: Minion, setStatus: MinionStatus): Promise<void | ErrorResponse> {
    ...
    try {
      ...
      logger.error(
        `[ModulesManager.getStatus] setting minion "${minion.minionId}" status failed error:"${error.message ||
          JSON.stringify(error)}"`,
      );
      logger.error(
        `[ModulesManager.getStatus] setting minion "${
          minion.minionId
        }" status failed set status attempt "${JSON.stringify(setStatusCopy)}}" minion full state "${JSON.stringify(
          minion,
        )}"`,
      );
      if (error instanceof TimeoutError) {
