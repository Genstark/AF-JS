...
export class TuyaHandler extends BrandModuleBase {
  ...
  public async refreshCommunication(): Promise<void> {
    for (const tuyaApi of Object.values(this.physicalDevicesMap)) {
      try {
        ...
        logger.warn(
          `[tuyaAPI.refreshCommunication] disconnecting device ${tuyaApi.mac} error ${error.message ||
  ...
  private async getTuyaDevice(minionDevice: MinionDevice): Promise<any> {
    ...
    if (tuyaApiObject) {
      ...
      try {
        ...
        logger.warn(
          `[tuyaAPI.getTuyaDevice] disconnecting device ${tuyaApiObject.mac} error ${error.message ||
            JSON.stringify(error)}`,
        );
      }
    }
    ...
    try {
      ...
      logger.warn(
        `[tuyaAPI.getTuyaDevice] Creating new Tuya api object failed ${tuyaApiObject.mac} error ${error.message ||
          JSON.stringify(error)}`,
      );
      try {
        await tuyaApiObject.disconnect();
        ...
        logger.warn(
          `[tuyaAPI.getTuyaDevice] Disconnecting new attempt failed ${tuyaApiObject.mac} error ${error.message ||
            JSON.stringify(error)}`,
        );
      }
    }
  ...
  private async watchDevice(tuyaDevice: TuyaDevice, minionDevice: MinionDevice) {
      ...
      if (minionDevice.model.includes('curtain')) {
        try {
          ...
          for (const minion of minions) {
            ...
            logger.debug(
              `[tuyaHandler] minion "${minion.minionId}" status arrived from device ${JSON.stringify(rowStatus)}`,
            );
      ...
      tuyaDevice.get({ schema: true }).then(status => {
        ...
        this.retrieveMinions.pull().then(minions => {
          for (const minion of minions) {
            ...
            logger.debug(
              `[tuyaHandler] minion "${minion.minionId}" status arrived from device ${JSON.stringify(status)}`,
            );
    ...
    tuyaDevice.on('error', async (err: any) => {
      logger.debug(
        `[tuyaHandler] tuya device mac: "${minionDevice.pysicalDevice.mac}" error: ${err.message} ${err.stack ||
          JSON.stringify(err)}`,
      );
